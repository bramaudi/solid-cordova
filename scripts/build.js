...
let html = fs.readFileSync(`${path.resolve()}/dist/index.html`, { encoding: 'utf-8' })
...
fs.writeFileSync(`${path.resolve()}/dist/index.html`, html, { encoding: 'utf-8' })
console.log('Replaced successfuly');

const unusedJs = matches[0].replace(/<script(.*?)src="(.*?)"(.*?)>[\s\S]*?<\/script>/g, '$2')
fs.rmSync(`${path.resolve()}/dist/${unusedJs}`)
